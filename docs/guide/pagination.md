...
然而，很多人低估了客户端可以处理的数据量。如果您的表格只有几千行或更少，客户端分页仍然是一个可行的选择。TanStack Table 设计为可以扩展到数万行，在分页、过滤、排序和分组方面具有良好的性能。[官方分页示例](../framework/react/examples/pagination) 加载了 100,000 行数据，仍然表现良好，尽管只有少数几列。
